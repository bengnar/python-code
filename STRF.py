...
bins_strf = np.arange(0, resp_dur, per_strf)
...
'''RESPONSE'''
f = h5py.File(os.path.join(basedir, 'voc_ko_nai_20130116', 'fileconversion', 'VOC001.h5'))
rast = f['site00']['rast'].value
...
alphas = np.logspace(0, 5, 45) # 10^0 to 10^5, 45 times
nBoots = 2 #25
...
for c in range(nchunks):
    ...
    Rresps = resps[Rinds, :] ## Selects rows of resps where Rinds is True (training)
    Presps = resps[Pinds, :] # test set
